...
document.getElementById("searchForm").addEventListener("submit", function (e) {
    ...
    const allMessages = document.querySelectorAll("#chatHistory .text");

    allMessages.forEach(msg => {
      const content = msg.textContent.toLowerCase();

      if (query && content.includes(query)) {
        msg.style.backgroundColor = "#ffff00"; // Highlight matching message
        msg.style.display = "block";           // Show matching message
      } else {
        msg.style.backgroundColor = "";        // Clear previous highlights
        msg.style.display = query ? "none" : "block"; // Hide non-matches if searching
      }
    });
  });
